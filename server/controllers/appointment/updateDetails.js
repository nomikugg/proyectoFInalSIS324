...
const createAppointmentHandler = async (req, res) => {
  ...
  try {
    ...
    if (!ifDoc || !ifPatient)
      return res
        .status(400)
        .json({ message: "Invalid Request : Doctor or patient doesnt exists" });
    ...
    if (
      ifAppointment &&
      ifAppointment.state !== "rejected" &&
      ifAppointment.state !== "closed"
    )
...
const createScheduleHandler = async (req, res) => {
  ...
  if (!appointmentId || !data.at)
    res.status(404).json({ message: "some fields are missing" });
  try {
    const appointment = await Appointment.findByPk(appointmentId);
    if (!appointment)
      return res.json({ message: "Appointment doesnt exists!" });
