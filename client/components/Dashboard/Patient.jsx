...
const ScheduleCard = ({ schedule }) => {
  ...
  return (
    <div
      onClick={() => router.push(`/appointments/${schedule.appointmentId}`)}
      className="bg-slate-100 dark:bg-darkElevation-300 rounded-md text-sm hover:bg-slate-200 transition-all cursor-pointer flex justify-between p-2 py-4"
    >
...
const MedicalRecordCard = ({ record }) => {
  return (
    <div className="p-2 bg-green-100 dark:bg-darkElevation-300 rounded-md">
      ...
      <div className="text-[0.65rem] mt-1">
...
const DianogisCard = ({ diagnosis }) => {
  ...
  return (
    <div
      ...
      className="p-2 bg-pink-100 cursor-pointer dark:bg-darkElevation-300 flex hover:bg-pink-50 justify-between items-center text-sm rounded-md"
    >
      <div className="flex space-x-5 items-center">
        <div className="p-2 rounded-md bg-pink-400">
          <AiFillRobot className="text-white" />
        </div>
...
const PatientDashboard = ({ patient, user, appointedDoctors }) => {
  ...
  const schdulesArray = patient => {
    ...
    patient.appointments
      ...
      .map(a => {
        const arr2 = a.schedules
          .filter(s => s.state === "future")
          .map(s => ({
            ...s,
            doctorId: a.doctor.uuid,
      ...
      <div className="flex space-x-10">
          ...
          {appointedDoctors.length > 0 ? (
            appointedDoctors.map(doc => (
              <DoctorCard
                ...
                speciality={
                  `${doc.specialities[0]?.title || "Unset"} Specialist` ||
                  "Not Found"
                }
              />
            ))
        ...
        >
          {patient.diagnoses.length > 0 ? (
            patient.diagnoses.map(diagnosis => (
              <div>
                <DianogisCard diagnosis={diagnosis} />
              </div>
            ))
          ) : (
        ...
        <InfoBox
          ...
          icon={
            <AiOutlinePlusSquare
              size={31}
              className="-mt-1 text-green-600 hover:scale-105"
            />
          }
        >
          {patient.medicalRecords.length > 0 ? (
            patient.medicalRecords.map(record => (
              <a href={record.file} target="true" className="block">
