    ...
    <Modal show={show} onClose={onClose} title={"Add Medical Record"}>
      <div className="mt-5 flex space-x-2 items-center">
        <button
          ...
          className="bg-[#5A5482] px-3 text-sm py-1 rounded-md text-white"
        >
          {loading ? "Loading..." : "Add File"}
        </button>
        <div className="text-sm text-gray-600*">
          {file ? file.name : "Add Medical Record"}
        </div>
        <input
          disabled={loading}
          ref={ref}
      ...
      <Input
        placeholder="Title of Medical Record (Eg : Blood Report)"
        label="title"
        value={title}
      ...
      <button
        onClick={handleSave}
        disabled={loading}
        className="mt-5 text-sm border-[#5A5482] border-2 rounded-md text-[#5A5482] hover:bg-[#5A5482] hover:text-white transition-all px-2 py-1  w-fit"
      >
