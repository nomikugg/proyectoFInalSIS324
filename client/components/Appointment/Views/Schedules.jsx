  ...
  return (
    <div className="w-full hover:bg-[#383c41] transition-all bg-[#222529] p-5 px-8 rounded-md flex justify-between items-center">
      ...
      <div className="flex items-center space-x-2">
        {" "}
        <MdOutlineAlarm />
        <div> {formatDate(schedule.at)}</div>
      </div>
      {state === "future" && (
        <div className="flex items-center space-x-2">
          <div className="p-2 rounded-md text-sm bg-[#2b2f34] cursor-pointer hover:bg-[#1415179e] transition-all flex space-x-1 items-center">
            <a
              href={`http://localhost:3001/room/${
                schedule.id
              }?name=${user.fullName.split(" ").join("")}`}
            >
              🚪ENTER
            </a>
          </div>
          <div
            className="p-2 rounded-md text-sm bg-[#2b2f34] cursor-pointer hover:bg-[#1415179e] transition-all"
            onClick={setStateDone}
          >
            ✅Done
          </div>
          <div className="p-2 rounded-md text-sm bg-[#2b2f34] cursor-pointer hover:bg-[#1415179e] transition-all">
            ⌚Reschedule
          </div>
        </div>
      )}
...
const SchduleCreator = ({
  ...
  const handleSubmit = async () => {
    const realDate = moment(creatorData.date + "T" + creatorData.time).format(
      "YYYY-MM-DDTHH:mm:ss.000Z"
    );
    try {
      const res = await createScheduleAPI(appointmentId, {
        at: realDate,
      ...
      <div className="grid grid-cols-[1.3fr_1.4fr_1.4fr_1fr] gap-10 w-full">
        <div className="">
          <input
            ...
            onChange={e =>
              setCreatorData({ ...creatorData, title: e.target.value })
            }
            type="text"
            className="w-full rounded-md p-[6px] px-2 bg-[#32353a]"
        ...
        <div className="">
          <input
            ...
            onChange={e =>
              setCreatorData({ ...creatorData, date: e.target.value })
            }
            type="date"
            className="w-full rounded-md p-[6px] px-2 text-slate-300 bg-[#32353a]"
          />
        </div>
        <div className="">
          <input
            value={creatorData.time}
            type="time"
            onChange={e =>
              setCreatorData({ ...creatorData, time: e.target.value })
            }
            className="w-full rounded-md p-[6px] px-2 text-slate-300 bg-[#32353a]"
          />
        </div>
        <div className="flex space-x-2 text-xs items-center">
          <div
            ...
            className="w-fit bg-primary p-2 rounded-md px-5 text-white bg-opacity-50"
          >
            Create
          </div>
          <div
            onClick={() => setCreatorData(initialState)}
            className="w-fit bg-red-600 p-2 rounded-md px-5 text-white bg-opacity-50"
          >
            Clear
          </div>
        </div>
  ...
  return (
    <>
      <div className="text-slate-400 p-4 h-[485px] overflow-y-auto">
        ...
        <div className="mt-4 space-y-4">
          {currentSchedules
            .filter(s => s.state === "past" || s.state === "done")
            .map(s => (
