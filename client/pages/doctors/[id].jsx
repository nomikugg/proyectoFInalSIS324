...
export const getServerSideProps = async ctx => {
  ...
  try {
    ...
    if (auth.decodedData.role === "patient") {
      let results = await getAllAppointments(
        auth.decodedData.additionalData.uuid
      );
      return {
        props: {
...
const Profile = ({ user, doctor, appointedDoctors }) => {
  ...
  const handleClick = async () => {
    try {
      ...
      const res = await createAppointmentAPI(
        user.additionalData.uuid,
        doctor.uuid
      );
      console.log(res);
      console.log("Aaaaaaaaaaaaaaaappointment creation successful:", res);
      setAppointmentState("pending");
      console.log(
        "Appointment state after update: oooooooooo",
        appointmentState
      );
    } catch (err) {
  ...
  const ActionButton = ({ role }) => {
    ...
    if (
      appointmentState &&
      (appointmentState === "rejected" || appointmentState === "closed")
    )
      appointmentState = false;
    if (appointmentState && appointmentState === "active")
      text = "Active Appointment";
    ...
    return (
      <>
        {role === "patient" && (
          <button
            disabled={appointmentState}
            onClick={handleClick}
            className={cls(
              "text-[0.7rem]  bg-opacity-90 hover:bg-opacity-100 w-fit p-1 border-[1px] text-white rounded-md px-2",
              { "bg-primary dark:bg-darkPrimary": !appointmentState },
              { "bg-gray-700": appointmentState === "pending" },
              { "bg-green-600": appointmentState === "active" }
            )}
          >
            {text}
          </button>
      ...
      <div className="bg-white dark:bg-darkElevation-100 w-full h-[250px] mt-9 rounded-xl flex items-center justify-between px-8 shadow-md">
        <div className="flex space-x-6 items-center">
          <div
            ...
            style={{
              background: `url(${doctor.user.img}) center center/cover`,
            }}
          ></div>
          <div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex space-x-1">
            <MdPhone size={20} />{" "}
            <p className="text-sm">{doctor.user.contact}</p>
          </div>
          <div className="flex space-x-1">
      ...
      <div className="mt-7 w-full flex space-x-12">
        <div className="h-[350px] flex flex-col items-center w-24 rounded-tl-xl rounded-bl-xl bg-white dark:bg-darkElevation-100">
          ...
          <div className="flex flex-col items-center justify-center p-3 border-b-2 hover:scale-110 transition-all cursor-pointer">
            <MdLocalMall
              size={32}
              className="text-primary dark:text-gray-700"
            />
