...
export const getServerSideProps = async ctx => {
  ...
  try {
    ...
    if (auth.decodedData.role === "doctor") {
      appointments = await getDoctorAppointmentsAPI(auth.decodedData.dataId);
    } else {
      ...
      >
        ...
        >
          <Link href={`/appointments/${appointment.id}`}>
            <div className="w-[40px] h-[40px] bg-darkSecondaryVariant rounded-full border-primary border-2"></div>
          </Link>
        ...
        <div className="flex space-x-2 text-xs items-center">
          ...
          <div
            onClick={() =>
              router.push(`${role === "doctor" ? linkPatient : linkDoctor}`)
            }
          >
            {role === "doctor" ? patient.user.fullName : doctor.user.fullName}
          </div>
        </div>
        <div className="flex space-x-2 text-xs items-center">
          <BiTimeFive />
        ...
        >
          ...
          {status === "closed" && (
            <IoMdDoneAll className="text-gray-700 dark:text-gray-300" />
          )}
          {status === "pending" && (
        ...
        <div className="flex space-x-5 text-xs items-center">
          {status === "active" && (
            <div
              ...
              className="hover:bg-gray-200 dark:hover:bg-darkElevation-900 transition-all bg-gray-400 dark:bg-darkElevation-700 dark:border-none dark:text-gray-300 border-[1px] w-fit p-2 rounded-md bg-opacity-10"
            >
              Close Case
            </div>
          )}
    ...
    <>
      <div className="mt-9 px-5">
        <div className="flex justify-between items-center">
          ...
          <div
            onClick={() => handleStateUpdate("rejected")}
            className="bg-primary dark:bg-darkPrimary dark:bg-opacity-40 py-1 px-3 rounded-lg cursor-pointer text-white"
          >
            {" "}
            + create
          </div>
